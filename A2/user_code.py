...
def plot_learning_curves(rewards_mc, rewards_td):
    """Plot learning curves for MC and TD rewards."""
    plt.figure(figsize=(12, 6))
    mc_smoothed = moving_average(rewards_mc)
    td_smoothed = moving_average(rewards_td)
    plt.plot(mc_smoothed, label='Monte Carlo', color='blue')
    plt.plot(td_smoothed, label='Q-Learning', color='orange')
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title(f'Learning Curves: Monte Carlo vs Q-Learning for epsilon={EPSILON}, gamma={GAMMA}, alpha={ALPHA}, num_bins={NUM_BINS}')
    plt.legend()
    plt.grid()
    plt.show()
...
def main():
    ...
    plot_learning_curves(rewards_mc, rewards_td) #uncomment after montecarlo
